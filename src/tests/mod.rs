...
macro_rules! generate_instruction_parser_tests {
    ($($name:ident: $mnemonic:tt, $am:tt, $opcode:expr, $cycles:expr,)*) => {
        $(
            #[test]
            fn $name() {
                let bytecode: [u8; 3] = [$opcode, 0x00, 0x00];
                assert_eq!(
                    Instruction::new(<$mnemonic>::default(), <$am>::default()),
                    Instruction::new(<$mnemonic>::default(), <$am>::default()).parse(&bytecode).unwrap().unwrap()
                )
            }
        )*
    };
...
#[rustfmt::skip]
generate_instruction_parser_tests!(
